"""
orchestration/airflow/dags/finance_ml_training_dag.py
-----------------------------------------------------
...
Requirements:
  ...
  - `src/` package path available on PYTHONPATH.
  - JAVA_HOME set for PySpark.
  - MLflow local tracking server in `artifacts/mlruns/` (created automatically).
"""
...
with DAG(
    ...
    def _build_ml_tables(**ctx):
        ...
        """
        from finance_mlops.pipelines.lakehouse.gold.build_ml_tables import run
    ...
    def _train_model(**ctx):
        ...
        from finance_mlops.pipelines.training.train_job import run
        from finance_mlops.pipelines.common.utils.spark import get_spark
    ...
    def _evaluate_and_register(**ctx):
        ...
        from finance_mlops.pipelines.training.evaluate_and_register_job import run
    ...
    def _export_serving_artifacts(**ctx):
        from finance_mlops.pipelines.training.export_model_for_serving_job import run
