...
from finance_mlops.config.paths import LAKEHOUSE_GOLD_DIR, LAKEHOUSE_DIR, PROJECT_ROOT
...
def verify(spark) -> bool:
    ...
    for layer, tables in EXPECTED_TABLES.items():
        for table_name, required_cols in tables.items():
            path = os.path.join(str(LAKEHOUSE_DIR), layer, table_name)
    ...
    # 4. OBT sample
    obt_path = os.path.join(str(LAKEHOUSE_GOLD_DIR), "obt_transaction_fraud_view")
    ...
    feat_path = os.path.join(str(LAKEHOUSE_GOLD_DIR), "feat_account_unified")
    ...
    health_path = os.path.join(str(LAKEHOUSE_GOLD_DIR), "agg_feature_health_daily")
    ...
    training_path = os.path.join(str(LAKEHOUSE_GOLD_DIR), "ml_fraud_detection_training")
