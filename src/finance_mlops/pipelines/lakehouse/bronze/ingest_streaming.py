...
(`artifacts/source/streaming/events_stream.jsonl`) and appends it as a
...
from finance_mlops.config.paths import LAKEHOUSE_BRONZE_DIR, SOURCE_STREAMING_FILE
from finance_mlops.pipelines.common.utils.spark import get_spark

log = logging.getLogger(__name__)

# ── Path configuration ────────────────────────────────────────────────────────
STREAMING_INPUT_PATH = str(SOURCE_STREAMING_FILE)
    ...
    try:
        if not os.path.exists(STREAMING_INPUT_PATH):
            raise FileNotFoundError(
                ...
                "Run `python -m finance_mlops.data_generator.main` first."
            )
