...
The job is designed for a local MLflow tracking backend (`file://artifacts/mlruns`) and
creates a deterministic artifact bundle under:

  artifacts/serving/<MODEL_NAME>/<version>/
...
from finance_mlops.config.paths import MLRUNS_DIR, REPORTS_DIR, SERVING_ROOT
...
SERVING_ROOT = Path(SERVING_ROOT) / MODEL_NAME
EVAL_REPORT_PATH = Path(REPORTS_DIR) / "ml_eval_report.json"
